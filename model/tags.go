...
*/
func (s *Store) ensureTags(tx *gorm.DB, ownerID uint, names []string) ([]Tag, error) {
	if len(names) == 0 {
		return nil, nil
	}
...
*/
// comments in English
func (s *Store) addTagsToParent(tx *gorm.DB, ownerID uint, parentType ParentType, parentID uint, tags []Tag) error {
	if len(tags) == 0 {
		return nil
	}
...
func (s *Store) replaceTagsForParent(
	tx *gorm.DB,
	ownerID uint,
	parentType ParentType,
	parentID uint,
	...
	if len(newTags) > 0 {
		...
		if err := s.DeleteUnusedTagsByIDs(tx, ownerID, removed); err != nil {
			return err
		}
		return nil
	}
	...
	if err := s.DeleteUnusedTagsByIDs(tx, ownerID, currentIDs); err != nil {
		return err
	}
	return nil
}
...
func (s *Store) AddTagsToCompanyByName(companyID, ownerID uint, names []string) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		tags, err := s.ensureTags(tx, ownerID, names)
		if err != nil {
			return err
		}
		return s.addTagsToParent(tx, ownerID, ParentTypeCompany, companyID, tags)
	})
}
...
func (s *Store) ReplaceCompanyTagsByName(companyID, ownerID uint, names []string) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		return s.replaceTagsForParent(tx, ownerID, ParentTypeCompany, companyID, tags)
	})
}
...
func (s *Store) AddTagsToPersonByName(personID, ownerID uint, names []string) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		return s.addTagsToParent(tx, ownerID, ParentTypePerson, personID, tags)
	})
}
...
func (s *Store) ReplacePersonTagsByName(personID, ownerID uint, names []string) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		return s.replaceTagsForParent(tx, ownerID, ParentTypePerson, personID, tags)
	})
}
...
func (s *Store) SuggestTagNames(ownerID uint, prefix string, limit int) ([]string, error) {
	tags, err := s.SuggestTags(ownerID, prefix, limit)
	if err != nil {
		return nil, err
	}
